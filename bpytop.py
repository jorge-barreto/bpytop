...
class NetCollector(Collector):
	...
	#min_top: int = 10<<10
	#* Stats structure = stats[network device][download, upload][total, last, top, graph_top, offset, speed, redraw, graph_raise, graph_low] = int, List[int], bool
...
class GpuCollector(Collector):
	...
	def _get_power(cls, card):
		power = lambda n: round(int(cls._read(cls._get_hwmon(card) + f"power{n}_average")) / 1000000, 2) # Watts
